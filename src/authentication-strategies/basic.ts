...
import {MyUserService} from './../services/jwt-authentication/user.service';
...
export class BasicStrategy implements AuthenticationStrategy {
  ...
  verify(
    ...
    this.userService.verifyCredentials({email: username, phone: username, password}).then((user) => {
    ...
    //     }
    //     const user = users[0];
    //     if (!user.credentials || user.password.hash !== password) {
    //       return done(null, false);
    //     }
